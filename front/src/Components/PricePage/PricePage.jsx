  ...
  return (
    <div className='price-page'>
      ...
      <div className="all-pakkets">
        ...
        {packets.map((a, i) => (
          <PacketsList 
            ...
            points={a.points} 
            prices={a.prices} 
            extras={a.extras}   // передаём дополнительную информацию
          />
        ))}
        <Dosp />
      </div>
      ...
      <div style={{ margin: "4vw auto", width: "50vw" }}>
        <ButtonLink 
          links={links.contact1} 
          text={"напишите нам для рассчета стоимости вашего праздника"} 
          color="#FFE300" 
          col="#ACA214"
        />
      </div>
...
export const PriseItem = (props) => {
  ...
  return (
    <div className="price-item2">
      {props.id === 0 && <img src={img2} alt="" className='spice-img1' />}
      {props.id === 1 && <img src={img3} alt="" className='spice-img2' />}
      {props.id === 1 && <img src={img3} alt="" className='spice-img3' />}
    ...
    <div className="new-Bloc">
      ...
      <div className="allBlocksPac">
        <div className="pac-lock1">
          <p className='packet-point'>Что входит:</p>
          {props.points.map((a, i) => (
            <p className='packet-point' key={i}>• {a.text}</p>
          ))}
        </div>
        <div className="pac-lock2">
          <p className='packet-point'>Стоимость пакета</p>
          {props.prices.map((a, i) => (
            <p className='packet-point' key={i}>{a.value}</p>
          ))}
        </div>
        {/* Динамический блок "Дополнительно" (extras) */}
        {props.extras && props.extras.length > 0 && (
          <div className="pac-lock3">
            <p className='packet-point'>Доплата за 1 ребенка свыше указанного колличества:</p>
            {props.extras.map((extra, i) => (
              <p className='packet-point' key={i}>• {extra.text}</p>
            ))}
          </div>
        )}
      </div>
    ...
    <>
      <div className="dops-item">
        ...
        <div className="dops-item-fl">
          ...
          <div className="drop-center">
            <p className="prices-sub-sub-text">Выходные и праздничные дни: </p>
            <p className="prices-sub-sub-text">1200 </p>
          </div>
        </div>
      </div>
    ...
    <div className='jaba'>
      <div className="wrapper">
        ...
        <div className="jaba-text">
          <p>Дорогие гости! Обращаем Ваше внимание - еду, напитки, одноразовую посуду и столовые приборы вы можете принести с собой. Предупреждайте ваших гостей о том что наш парк посещается только в носочках (без бахил или сменной обуви).</p>
        </div>
      </div>
    </div>
  );
